...
func (r *SQLTaskRepository) GetTaskByID(ctx context.Context, id uint) (*models.Task, error) {
	query := "SELECT id, name, description, status, created_at, updated_at, user_id FROM tasks WHERE id = $1"
	var task models.Task
	err := r.db.QueryRow(ctx, query, id).Scan(&task.ID, &task.Name, &task.Description, &task.Status, &task.CreatedAt, &task.UpdatedAt, &task.UserID)
	if err != nil {
...
func (r *SQLTaskRepository) UpdateTask(ctx context.Context, task *models.Task) error {
	query := "UPDATE tasks SET name = $1, description = $2, status = $3, updated_at = $4 WHERE id = $5"
	_, err := r.db.Exec(ctx, query, task.Name, task.Description, task.Status, time.Now(), task.ID)
	if err != nil {
...
func (r *SQLTaskRepository) DeleteTask(ctx context.Context, id uint) error {
